...
public class MemberDao {
    ...
    public void updateMember(Member member) {
        try (
            ...
            String str = "Hello";
            str += " JDBC"; // str에 "JDBC" 추가가 아니라 기존 문자열은 버려지고 새로운 String 객체가 저장됨(주소값 달라짐)

            StringBuilder sql = new StringBuilder("UPDATE members SET ");

            if (updateName) {
                sql.append("name = ?, "); // 이후의 Email 추가 여부가 미정이라서 , " 첨부
            }
            ...
            // SQL 쿼리 실행
            PreparedStatement pstmt = conn.prepareStatement(sql.toString()); // StirngBuilder라서 toString 필요
